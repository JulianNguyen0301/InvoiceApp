...
def pdf_to_png():
    pdf_folder = r"H:/APP UNIVERSITY/CODE PYTHON/OpenCV-Master-Computer-Vision-in-Python/SourcecodeOCR/PDF_scan_color"
    saving_folder = r"H:/APP UNIVERSITY/CODE PYTHON/OpenCV-Master-Computer-Vision-in-Python/SourcecodeOCR/Source_Compare_scan_color"
...
def del_space(result):
    ...
    for i in range(len(result["conf"])):
        if result["conf"][i] == -1 or result["text"][i] == '' or result["text"][i] == ' ':
...
def convert_png_to_pdf():
    input_folder = "SourcecodeOCR\Source_Compare_scan_color"
    output_folder = "SourcecodeOCR/PDF_scan_color"
...
def invoice_cocacola(img,dem,tkh,dc,mst,ms,kh,shd,ngay):
    ...
    roi2 = [[(12, 158), (63, 343), 'text', 'STT'], 
            ...
            [(1127, 155), (1245, 346), 'text', 'Đơn giá'], 
    ...
    for z,r in enumerate(roi2):
        ...
        if r[3] == 'Số tiền viết bằng chữ':
            full_text = text_return_fulltext(imgCrop)
            sht.range("E7").value = r[3]
            sht.range("F7").value = full_text
    
    file_name = f"Cocacola_C_{dem}.xlsx"
...
def invoice_linhkhoa(img,dem,tkh,dc,mst,ms,kh,shd,ngay):
    ...
    for x,r in enumerate(roi2):
        ...
        if r[3] == 'STT':
            _, imgCrop = cv2.threshold(imgCrop, 128, 255, cv2.THRESH_TRUNC)
            kernel = np.array([[-1, -1, -1],
                    [-1,  10, -1],
                    [-1, -1, -1]])
            ...
            imgCrop = cv2.convertScaleAbs(imgCrop, alpha=0.8, beta=0)
            ...
            output_list = [[item.replace('.', ',') for item in sublist] for sublist in rows]
            sht.range("D9").value = r[3]
            for index, row in enumerate(output_list, start=10):
                sht.range("D" + str(index)).number_format = "@"
                sht.range("D" + str(index)).value = row

        elif r[3] == "Đơn giá":
            imgCrop = cv2.resize(imgCrop,None,fx=0.8,fy=0.8,interpolation=cv2.INTER_BITS)
            imgCrop = cv2.GaussianBlur(imgCrop, (5, 5), 0)
            result_2_1 = pytesseract.image_to_data(imgCrop, lang='eng', config='tessdata', output_type=Output.DICT)
            rows = del_space(result_2_1)
            sht.range("E9").value = r[3]
            ...
            for index, row in enumerate(rows, start=10):
                sht.range("F" + str(index)).number_format = "#,##0"
                text_value = row
                numeric_value = float("".join(text_value).replace('.', '').replace(',', ''))
                sht.range("F" + str(index)).value = numeric_value
        elif r[2] == "HH":
            _, imgCrop = cv2.threshold(imgCrop, 128, 255, cv2.THRESH_BINARY)
            imgCrop = cv2.convertScaleAbs(imgCrop, alpha=0.8, beta=0)
    ...
    roi3 = [[(253, 145), (1210, 190), 'text', 'Số tiền viết bằng chữ']]
    ...
    for index, line in enumerate(selected_numbers,start = 4):
                sht.range("F" + str(index)).number_format = "#,##0"
                text_value = line
    ...
    for z,r in enumerate(roi3):
        ...
        if r[3] == 'Số tiền viết bằng chữ':
            ...
            sht.range("E7").value = r[3]
            sht.range("F7").value = full_text               
    file_name = f"LinhKhoa_C_{dem}.xlsx"
    wb.save(fr'SourcecodeOCR\Data_Excel\{file_name}')  
    excel_file_path = fr"SourcecodeOCR\Data_Excel\{file_name}" 
    make_columns_bold(excel_file_path, 1,7,1)
    make_columns_bold(excel_file_path, 4,7,5)
...
def main():
    ...
    path_compare = 'SourcecodeOCR\Source_Compare_scan_color'
    ...
    for img1 in myList_compare:
        ...
        if finalVal == 0: 
            ...
            element1 = []
            for element in data1['text']:
                if is_date_format(element):
            ...
            while True:
                    ...
                    if text_verify == "NNT tạm nghỉ kinh doanh có thời hạn":
                        ...
                        break
            os.remove(img_path)
            invoice_cocacola(imgScan, dem_0,lines[0],lines[2],number_elements[0],number_elements[1],elements_with_numbers[1],elements_with_numbers[0],element1[0])
        elif finalVal == 1: 
            img_path = os.path.join(path_compare, myList_compare[index_1])
            ...
            None
    convert_png_to_pdf()
    folder_path = r"SourcecodeOCR\Source_Compare_scan_color"
    clear_image_files(folder_path)        
    file_count = len(os.listdir("SourcecodeOCR\PDF_scan_color"))
    print(f"Số lượng hóa đơn không thể trích xuất là: {file_count}")
    for filename in os.listdir("SourcecodeOCR\PDF_scan_color"):
        print(filename)
    Excel_Jusified()
